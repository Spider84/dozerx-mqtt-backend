...
def process_mqtt_message(topic, payload):
    with get_db_session() as db:
        parts = topic.split('/')
        if len(parts) < 3 or parts[0] != "dozerx": return
        mac, action = parts[1], parts[2]
        logger.debug(f"Processing MQTT message: {topic} -> {action} for device {mac}")

        device = db.query(DBDevice).filter(DBDevice.mac == mac).first()
        if not device:
            logger.info(f"Creating new device record for MAC: {mac}")
            device = DBDevice(mac=mac)
            db.add(device)
            db.flush()  # Flush to get ID without committing yet
            db.refresh(device)
            logger.info(f"Created device with MAC: {mac}")
        else:
            logger.debug(f"Found existing device: {mac}")

        if action == "status":
            logger.debug(f"Received status message for device {mac}: {payload}")
            try:
                ...
                for btn in data.get("buttons", []):
                    num, val = btn.get("num"), btn.get("cnt", 0)
                    if 1 <= num <= 6:
                        ...
                        if val != current_cnt:
                            ...
                            logger.debug(f"Updated button {num} count: {current_cnt} -> {val}")
                    else:
                        logger.warning(f"Invalid button number: {num} for device {mac}")
                ...
                logger.debug(f"Device {mac} updated fields: {updated_fields}")
                
                history_entry = DBHistory(mac=mac, event_type="status", event_data=payload)
                db.add(history_entry)
                logger.debug(f"Added history entry for device {mac}: status")
                
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse JSON payload for device {mac}: {e}")
            ...
            logger.debug(f"Button event for device {mac}: {btn_event} (stop: {is_stop})")
            
            history_entry = DBHistory(mac=mac, event_type=event_type, event_data=btn_event)
            db.add(history_entry)
            logger.debug(f"Added history entry for device {mac}: {event_type}")

            if not is_stop:
                if btn_event == "long":
                    device.long_cnt += 1
                    device.global_long_cnt += 1
                    logger.debug(f"Incremented long press count for device {mac}")
                elif btn_event.isdigit():
                    num = int(btn_event)
                    if num == 1: device.btn1_cnt += 1; device.global_btn1 += 1
                    elif num == 2: device.btn2_cnt += 1; device.global_btn2 += 1
                    elif num == 3: device.btn3_cnt += 1; device.global_btn3 += 1
                    elif num == 4: device.btn4_cnt += 1; device.global_btn4 += 1
                    elif num == 5: device.btn5_cnt += 1; device.global_btn5 += 1
                    elif num == 6: device.btn6_cnt += 1; device.global_btn6 += 1
                    device.total_cnt += 1
                    device.global_total_cnt += 1
                    logger.debug(f"Incremented button {num} count for device {mac}")
        
        logger.debug(f"Database changes will be committed by context manager for device {mac}")

mqtt_client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2)
if config['mqtt'].get('user'): mqtt_client.username_pw_set(config['mqtt']['user'], config['mqtt'].get('password'))
