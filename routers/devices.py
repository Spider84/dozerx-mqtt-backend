...
@router.get("/", summary="Get all devices", description="Retrieve MAC addresses of all devices or filter by client")
@limiter.limit(RATE_LIMITS["read"])  # Read operations can be more frequent
def get_devices(request: Request, client_id: int = None, db: Session = Depends(get_db), _: str = Depends(api_key_auth)):
    """
    ...
    Raises:
        ...
        500: Database error
    """
    logger.info(f"Getting device MAC addresses with client_id={client_id}")
    try:
        ...
        if client_id is not None:
            ...
            logger.debug(f"Applied filter: devices for client {client_id}")
        # If client_id is None, return all devices (default behavior)
        
        mac_addresses = [mac[0] for mac in query.all()]
        logger.info(f"Found {len(mac_addresses)} device MAC addresses")
        return mac_addresses
    except Exception as e:
        logger.error(f"Error retrieving device MAC addresses: {e}")
        raise
...
def create_device(request: Request, device: schemas.DeviceCreate, db: Session = Depends(get_db), _: str = Depends(api_key_auth)):
    """
    Create a new device.
    ...
    Raises:
        ...
        500: Database error
    """
    logger.info(f"Creating new device with MAC: {device.mac}")
    try:
        
        existing_device = db.query(models.DBDevice).filter(models.DBDevice.mac == device.mac).first()
        if existing_device: 
            logger.warning(f"Device with MAC {device.mac} already exists with ID: {existing_device.id}")
        ...
        logger.debug(f"Converted device data for database: {list(db_data.keys())}")
        
        db_dev = models.DBDevice(**db_data)
        db.add(db_dev)
        logger.debug(f"Added device {device.mac} to database session")
        db.commit()
        logger.debug(f"Database commit completed for device {device.mac}")
        db.refresh(db_dev)
        logger.info(f"Created device with MAC: {device.mac}")
        return db_dev
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error creating device {device.mac}: {e}")
        db.rollback()
        logger.debug(f"Database rollback for device creation {device.mac}")
        raise

@router.get("/{mac}", summary="Get device details", description="Get detailed information about a specific device with grouped data")
@validate_mac_param()
@limiter.limit(RATE_LIMITS["read"])  # Read operations
def get_device_detail(request: Request, mac: str, db: Session = Depends(get_db), _: str = Depends(api_key_auth)):
    """
    ...
    Raises:
        ...
        500: Database error
    """
    logger.info(f"Getting detailed device information for MAC: {mac}")
    try:
        ...
        logger.info(f"Retrieved detailed information for device {mac}")
        return device_detail
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error retrieving device details for {mac}: {e}")
        raise
...
def has_device_changes(db_device: models.DBDevice, update_data: dict) -> bool:
    ...
    for field, new_value in update_data.items():
        ...
        if current_value is None and new_value is None:
            ...
            logger.debug(f"Field {field} changed: {current_value} -> {new_value}")
            return True
    
    return False

@router.get("/{mac}/history", summary="Get device history", description="Retrieve MQTT message history for a specific device")
@validate_mac_param()
@limiter.limit(RATE_LIMITS["read"])  # Read operations
def get_device_history(
    ...
    Raises:
        ...
        500: Database error
    """
    logger.info(f"Getting history for device {mac} (limit={limit}, offset={offset}, event_type={event_type})")
    try:
        ...
        if event_type:
            ...
            logger.debug(f"Applied event_type filter: {event_type}")
        
        # Get total count for pagination info
        total_count = query.count()
        
        # Apply pagination and ordering (newest first)
        history_entries = query.order_by(models.DBHistory.timestamp.desc()).offset(offset).limit(limit).all()
        ...
        logger.info(f"Retrieved {len(history_list)} history entries for device {mac} (total: {total_count})")
        ...
        logger.error(f"Error retrieving history for device {mac}: {e}")
        raise

@router.put("/{mac}", summary="Update device", description="Update device settings with grouped structure")
@validate_mac_param()
@limiter.limit(RATE_LIMITS["write"])  # Write operations
def update_device(request: Request, mac: str, device: schemas.DeviceUpdate, db: Session = Depends(get_db), _: str = Depends(api_key_auth)):
    """
    ...
    try:
        ...
        if not update_data:
            logger.info(f"No fields to update for device {mac}")
            return db_dev
        
        # Check if any fields actually changed
        if not has_device_changes(db_dev, update_data):
            logger.info(f"No actual changes detected for device {mac} - skipping database update")
            return db_dev
        
        logger.info(f"Updating fields for device {mac}: {list(update_data.keys())}")
        for k, v in update_data.items(): 
            setattr(db_dev, k, v)
            logger.debug(f"Set {k} = {v} for device {mac}")
        
        db.commit()
        logger.debug(f"Database commit completed for device update {mac}")
        db.refresh(db_dev)
        logger.info(f"Successfully updated device with MAC: {mac}")
        return db_dev
    except HTTPException:
        raise
    except Exception as e:
        ...
        db.rollback()
        logger.debug(f"Database rollback for device update {mac}")
        raise
